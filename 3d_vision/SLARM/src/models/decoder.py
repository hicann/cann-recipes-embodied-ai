# coding=utf-8
# Adapted from
# https://github.com/NVlabs/GaussianSTORM/blob/main/storm/models/decoder.py
...
def check_results(result_dict) -> bool:
    if "rgb_key" not in result_dict:
        raise ValueError("rgb_key not found in result_dict")
    if "depth_key" not in result_dict:
        raise ValueError("depth_key not found in result_dict")
    if "alpha_key" not in result_dict:
        raise ValueError("alpha_key not found in result_dict")
    if "flow_key" not in result_dict:
        raise ValueError("flow_key not found in result_dict")
    if "decoder_depth_key" not in result_dict:
        raise ValueError("decoder_depth_key not found in result_dict")
    if "decoder_alpha_key" not in result_dict:
        raise ValueError("decoder_alpha_key not found in result_dict")
    if "decoder_flow_key" not in result_dict:
        raise ValueError("decoder_flow_key not found in result_dict")
    return True
...
class ConvDecoder(nn.Module):
    def __init__(
        ...
        for i, out_chans in enumerate(channels):
            for _ in range(num_res_blocks):
    ...
    def forward(self, render_results) -> Dict[str, Tensor]:
        required_keys = ["rendered_image", "rendered_alpha"]
        for key in required_keys:
            if key not in render_results:
                raise ValueError(f"{key} not found in render_results")

# coding=utf-8
# Adapted from
# https://github.com/NVlabs/GaussianSTORM/blob/main/storm/models/embedders.py
...
@dataclass
class RayComputationInputs:
    intrinsics: Tensor
    camtoworlds: Tensor
    intrinsics_shape: torch.Size
...
def equivalent_cross(a, b):
    ...
    Requires the last dimension of a and b to be 3. Using this equivalent
    implementation on Ascend devices instead of torch.cross achieves 900ms
    speedup for a single Cross op. The equivalent implementation has identical
    accuracy to torch.cross (error < 1e-9).
    """
...
class PluckerEmbedder(nn.Module):
    ...
    def forward(
        ...
        if intrinsics.shape[-2:] != (3, 3):
            raise ValueError(
                f"intrinsics should be (..., 3, 3), got {intrinsics.shape[-2:]}"
            )
        if camtoworlds.shape[-2:] != (4, 4):
            raise ValueError(
                f"camtoworlds should be (..., 4, 4), got {camtoworlds.shape[-2:]}"
            )
        ...
        intrinsics, grid_size, x_coords, y_coords = self._resolve_coords(
            intrinsics, image_size, patch_size
        )

        ray_inputs = RayComputationInputs(
            intrinsics=intrinsics,
            camtoworlds=camtoworlds,
            intrinsics_shape=intrinsics_shape,
            grid_size=grid_size,
            x_coords=x_coords,
            y_coords=y_coords,
        )
        ray_dict = self._compute_rays(ray_inputs)
        return ray_dict

    def _resolve_coords(self, intrinsics, image_size, patch_size):
        """Determine grid size, pixel-centre coordinates, and scaled intrinsics.

        Returns (intrinsics, grid_size, x_coords, y_coords) where intrinsics
        may be a scaled clone when the grid differs from the default.
        """
        image_size = to_2tuple(image_size) if image_size is not None else self.img_size
        patch_size = to_2tuple(patch_size) if patch_size is not None else self.patch_size
        ...
        if grid_size != self.grid_size:
            ...
            x_coords = x_coords.float().reshape(1, -1) + 0.5
            y_coords = y_coords.float().reshape(1, -1) + 0.5
            x_coords = x_coords.to(intrinsics.device)
            y_coords = y_coords.to(intrinsics.device)
            ...
            x_coords, y_coords = self.x, self.y

        return intrinsics, grid_size, x_coords, y_coords

    def _compute_rays(self, ray_inputs):
        """Compute origins, viewdirs, directions and plucker coords."""
        intrinsics = ray_inputs.intrinsics
        camtoworlds = ray_inputs.camtoworlds
        grid_size = ray_inputs.grid_size
        intrinsics_shape = ray_inputs.intrinsics_shape
        x_coords = ray_inputs.x_coords
        y_coords = ray_inputs.y_coords

        x_coords = x_coords.repeat(intrinsics.size(0), 1)
        y_coords = y_coords.repeat(intrinsics.size(0), 1)

        camera_dirs = torch.nn.functional.pad(
            torch.stack(
                [
                    (x_coords - intrinsics[:, 0, 2][..., None])
                    / intrinsics[:, 0, 0][..., None],
                    (y_coords - intrinsics[:, 1, 2][..., None])
                    / intrinsics[:, 1, 1][..., None],
                ],
                dim=-1,
            ),
        ...
        directions = torch.sum(
            camera_dirs[:, :, None, :] * camtoworlds[:, None, :3, :3], dim=-1
        )
        origins = torch.broadcast_to(
            camtoworlds[:, :3, -1].unsqueeze(1), directions.shape
        )

# coding=utf-8
# Adapted from
# https://github.com/NVlabs/GaussianSTORM/blob/main/storm/visualization/visualization_tools.py
...
def depth_visualizer(frame, opacity):
    """Visualize depth frames using turbo colormap with log scaling."""
...
def flow_visualizer(frame):
    """Visualize scene flow frames and convert to numpy array."""
    return (
        scene_flow_to_rgb(
            frame,
            background="bright",
...
def visualize_depth(x, acc=None, lo=None, hi=None, depth_curve_fn=lambda x: -np.log(x + 1e-6)):
    ...
    if colormap:
        ...
        if len(value.shape) != 3 or value.shape[-1] != 3:
            raise ValueError(
                f"Expected value to have shape (H, W, 3) when colormap is None, "
...
def scene_flow_to_rgb(flow: torch.Tensor, flow_max_radius: Optional[float] = None,
                      background: Optional[str] = "bright") -> Union[torch.Tensor, np.ndarray]:
    ...
    angle_fractional, angle_floor, angle_ceil = (op.fmod(angle, 1), op.trunc(angle), op.ceil(angle),)
    angle_fractional = angle_fractional[..., None]
    if backend == "torch":
        _wheel = wheel.to(angle_floor.device)
        float_hue = (_wheel[angle_floor.long()] * (1 - angle_fractional) + _wheel[angle_ceil.long()] * angle_fractional)
    else:
        float_hue = (wheel[angle_floor.astype(op.int64)] * (1 - angle_fractional) + wheel[
            angle_ceil.astype(op.int64)] * angle_fractional)
    ColorizationArgs = namedtuple("ColorizationArgs",
                                  ["move_hue_valid_radius", "move_hue_oversized_radius", "invalid_color"])

    if background == "dark":
        parameters = ColorizationArgs(move_hue_on_v_axis, move_hue_on_s_axis, op.array([255.0, 255.0, 255.0]))
    else:
    ...
    colors[oversized_radius_mask] = parameters.move_hue_oversized_radius(float_hue[oversized_radius_mask],
...
def get_robust_pca(features: Tensor, m: float = 2, remove_first_component=False):
    if len(features.shape) != 2:
        raise ValueError(
            f"Expected features to have shape (N, C), but got shape {features.shape}"
        )
...
def get_similarity_map(features: Tensor, img_size=(224, 224)):
    ...
    if len(features.shape) != 4:
        raise ValueError(
            f"Expected features should be (1, C, H, W), but got shape {features.shape}"
        )

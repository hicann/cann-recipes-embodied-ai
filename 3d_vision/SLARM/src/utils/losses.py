# coding=utf-8
# Adapted from
# https://github.com/NVlabs/GaussianSTORM/blob/main/storm/utils/losses.py
...
@dataclass
class GradientLossConfig:
    gradient_loss_fn: Optional[Callable] = None
...
class ConfidenceConfig:
    high_conf: float = 0.8
    low_conf: float = 0.2
    no_gt_conf: float = 1.0


@dataclass
class LossContext:
    lpips_loss: Optional[Callable] = None
    data_iter_step: Optional[int] = None
...
class PointLossConfig:
    ...
    gradient_loss_fn: Optional[Callable] = None
    valid_range: float = -1.0
    camera_centric_reg: float = -1.0
    disable_conf: bool = False
    all_mean: bool = False
    conf_loss_type: str = "v1"


@dataclass
class RegAndFlowLossContext:
    """Encapsulates parameters needed by _compute_reg_and_flow_loss."""
    gs_params: dict
    device: torch.device
    args: object
    pred_dict: dict
...
def normal_loss(prediction, target, mask, conf=None, loss_weights=None):
    """
    ...
    Args:
        ...
        loss_weights: LossWeights with gamma, alpha, and cos_eps for numerical stability
    """
    if loss_weights is None:
        loss_weights = LossWeights()
    gamma = loss_weights.gamma
    alpha = loss_weights.alpha
    cos_eps = loss_weights.cos_eps
...
def gradient_loss(prediction, target, mask, conf=None, loss_weights=None):
    """
    ...
    Args:
        ...
        mask: (B, H, W) valid pixel mask
        conf: (B, H, W) confidence weights (optional)
        loss_weights: LossWeights with gamma and alpha for confidence weighting
    """
    if loss_weights is None:
        loss_weights = LossWeights()
    gamma = loss_weights.gamma
...
def depth_loss(
    depth, depth_conf, batch,
    loss_weights=None, depth_loss_config=None
):
    if loss_weights is None:
        loss_weights = LossWeights()
    gamma = loss_weights.gamma
    alpha = loss_weights.alpha
    if depth_loss_config is None:
        depth_loss_config = DepthLossConfig()
    gradient_loss_fn = depth_loss_config.gradient_loss_fn
    valid_range = depth_loss_config.valid_range
    loss_type = depth_loss_config.loss_type
    affine_inv = depth_loss_config.affine_inv
    disable_conf = depth_loss_config.disable_conf
    all_mean = depth_loss_config.all_mean
    ...
    if loss_type == "conf":
        conf_loss_dict = conf_loss(
            ...
            gamma=gamma, alpha=alpha, affine_inv=affine_inv,
        gradient_loss=gradient_loss_fn, valid_range=valid_range,
        postfix="_depth", disable_conf=disable_conf, all_mean=all_mean
        )
    else:
...
def point_loss(
    pts3d, pts3d_conf, batch,
    loss_weights=None, point_loss_config=None
):
    if loss_weights is None:
        loss_weights = LossWeights()
    gamma = loss_weights.gamma
    alpha = loss_weights.alpha
    if point_loss_config is None:
        point_loss_config = PointLossConfig()
    gradient_loss_fn = point_loss_config.gradient_loss_fn
    valid_range = point_loss_config.valid_range
    normalize_gt = point_loss_config.normalize_gt
    normalize_pred = point_loss_config.normalize_pred
    affine_inv = point_loss_config.affine_inv
    camera_centric_reg = point_loss_config.camera_centric_reg
    disable_conf = point_loss_config.disable_conf
    all_mean = point_loss_config.all_mean
    conf_loss_type = point_loss_config.conf_loss_type
    """
    pts3d: B, S, H, W, 3
    pts3d_conf: B, S, H, W
    """
    ...
    conf_loss_dict = conf_loss_fn(
        ...
        batch, normalize_gt=normalize_gt, normalize_pred=normalize_pred,
        gamma=gamma, alpha=alpha, affine_inv=affine_inv,
        gradient_loss=gradient_loss_fn, valid_range=valid_range,
...
def check_and_fix_inf_nan(input_tensor, loss_name="default", hard_max=100):
    ...
    if has_inf_nan:
        logger.warning(f"Tensor {loss_name} contains inf or nan values. "
...
def gradient_loss_multi_scale_wrapper(prediction, target, mask, scales=4, grad_config=None):
    """
    ...
    Args:
        ...
        grad_config: GradientLossConfig with gradient_loss_fn and conf
    """
    if grad_config is None:
        grad_config = GradientLossConfig()
    gradient_loss_fn = grad_config.gradient_loss_fn
    conf = grad_config.conf
    total = 0
...
def torch_quantile(
    ...
    *,
    ...
    Arguments:
        input_tensor (torch.Tensor): See torch.quantile.
    ...
    try:
        ...
        raise ValueError(f"Only scalar input 0<=q<=1 is currently supported (got {q})!") from e
    if not 0 <= q <= 1:
        raise ValueError(f"Only scalar input 0<=q<=1 is currently supported (got {q})!")

    if dim_was_none := dim is None:
        dim = 0
        input_tensor = input_tensor.reshape((-1,) + (1,) * (input_tensor.ndim - 1))
    ...
    # Compute k-th value
    k = inter(q * (input_tensor.shape[dim] - 1)) + 1
    out = torch.kthvalue(input_tensor, k, dim, keepdim=True, out=out)[0]
...
def regression_loss(pred, gt, mask, loss_weights=None, grad_config=None):
    """
    ...
    Args:
        ...
        loss_weights: LossWeights with gamma and alpha
        grad_config: GradientLossConfig with gradient_loss_fn, conf, valid_range
    ...
    conf = grad_config.conf
    gradient_loss_fn = grad_config.gradient_loss_fn
    valid_range = grad_config.valid_range
    ...
    if "normal" in gradient_loss_fn:
        ...
        loss_grad = gradient_loss_multi_scale_wrapper(
            ...
            scales=3,
            grad_config=GradientLossConfig(gradient_loss_fn=normal_loss, conf=to_feed_conf),
        )
        ...
        loss_grad = gradient_loss_multi_scale_wrapper(
            ...
            gt.reshape(bb * ss, hh, ww, nc),
            mask.reshape(bb * ss, hh, ww),
            grad_config=GradientLossConfig(gradient_loss_fn=gradient_loss, conf=to_feed_conf),
        )
...
def compute_depth_loss_with_conf(
    predictions, batch, loss_weights=None,
    gradient_loss_fn=None, valid_range=-1, **kwargs
):
    """
    Compute depth loss.
    ...
    loss_conf, loss_grad, loss_reg = regression_loss(
        ...
        loss_weights=LossWeights(gamma=gamma, alpha=alpha),
        grad_config=GradientLossConfig(gradient_loss_fn=gradient_loss_fn, conf=pred_depth_conf, valid_range=valid_range)
    )
...
def compute_point_loss_with_conf(
    predictions, batch, loss_weights=None,
    gradient_loss_fn=None, valid_range=-1, **kwargs
):
    """
    ...
    loss_conf, loss_grad, loss_reg = regression_loss(
        ...
        loss_weights=LossWeights(gamma=gamma, alpha=alpha),
        grad_config=GradientLossConfig(
            gradient_loss_fn=gradient_loss_fn, conf=pred_points_conf, valid_range=valid_range
        )
    )
...
def _prepare_camera_gt(batch_data, pose_encoding_type):
    """Prepare ground truth pose encoding and valid frame mask for camera loss."""
    ...
    # extrinsic is world to camera [b, s, 3, 4], intrinsic (no normalization) [b, s, 3, 3]
    gt_extrinsics = rearrange(
        batch_data['context_camtoworlds'], 'b t v ... -> b (t v) ...'
    ).inverse()[..., :3, :4]
    ...
    return gt_pose_encoding, valid_frame_mask


def _compute_camera_stage_loss(pred_pose_stage, gt_pose_encoding, valid_frame_mask,
                               loss_type, stage_weight):
    """Compute weighted camera loss for a single prediction stage."""
    ...
    # Return weighted losses for this stage
    return loss_t_stage * stage_weight, loss_r_stage * stage_weight, loss_fl_stage * stage_weight
...
def compute_camera_loss(
    ...
    camera_loss_weights=None,
    camera_config=None,
    **kwargs
):
    if camera_loss_weights is None:
        camera_loss_weights = CameraLossWeights()
    weight_trans = camera_loss_weights.weight_trans
    weight_rot = camera_loss_weights.weight_rot
    weight_focal = camera_loss_weights.weight_focal
    if camera_config is None:
        camera_config = CameraConfig()
    gamma = camera_config.gamma
    pose_encoding_type = camera_config.pose_encoding_type
    ...
    # Prepare ground truth pose encoding and valid frame mask
    gt_pose_encoding, valid_frame_mask = _prepare_camera_gt(batch_data, pose_encoding_type)
    ...
    for stage_idx in range(n_stages):
        ...
        loss_t_stage, loss_r_stage, loss_fl_stage = _compute_camera_stage_loss(
            pred_pose_stage, gt_pose_encoding, valid_frame_mask, loss_type, stage_weight
        )
        # Accumulate weighted losses across stages
        total_loss_t += loss_t_stage
        total_loss_r += loss_r_stage
        total_loss_fl += loss_fl_stage
...
def adaptive_confidence(gt_depth, pseudo_depth, alpha=0.1, valid_min=0.01, conf_config=None):
    """
    ...
    Args:
        ...
        conf_config: ConfidenceConfig with high_conf, low_conf, no_gt_conf
    """
    if conf_config is None:
        conf_config = ConfidenceConfig()
    high_conf = conf_config.high_conf
    low_conf = conf_config.low_conf
    no_gt_conf = conf_config.no_gt_conf
...
def _compute_context_rgb_loss(loss_dict, output_dict, input_dict, args, lpips_loss):
    # context rgb loss
...
def _compute_context_sky_opacity_loss(loss_dict, output_dict, input_dict, args):
    # context sky opacity loss
    if args.enable_context_sky_opacity_loss and \
       "context_sky_masks" in input_dict and \
       "rendered_context_alpha" in output_dict:
...
def _compute_context_depth_loss(loss_dict, output_dict, input_dict, args):
    # context depth loss
    if args.enable_context_depth_loss and "pred_context_depth" in output_dict and "context_depth" in input_dict:
        if args.context_depth_loss_with_conf:
            # conf loss
            context_depth_loss_dict = compute_depth_loss_with_conf(
                output_dict, input_dict,
                gradient_loss_fn='grad', valid_range=0.98
            )
            context_depth_loss = (
                context_depth_loss_dict["loss_conf_depth"] +
                context_depth_loss_dict["loss_reg_depth"] +
                context_depth_loss_dict["loss_grad_depth"]
            )
...
def _compute_context_point_loss(loss_dict, output_dict, input_dict, args):
    # context point loss
    if args.enable_context_point_loss and "pred_context_pts3d" in output_dict and "context_pts3d" in input_dict:
        if args.context_point_loss_with_conf:
            # conf loss
            context_point_loss_dict = compute_point_loss_with_conf(
                output_dict, input_dict,
                gradient_loss_fn='normal', valid_range=0.98
            )
            context_point_loss = (
                context_point_loss_dict["loss_conf_point"] +
                context_point_loss_dict["loss_reg_point"] +
                context_point_loss_dict["loss_grad_point"]
            )
            ...
            context_point_loss = F.l1_loss(
                pred_context_pts3d[gt_context_points_mask],
                gt_context_pts3d[gt_context_points_mask]
            )
            context_point_loss = args.context_point_loss_coeff * context_point_loss
            loss_dict["context_point_loss"] = context_point_loss


def _compute_context_feat_and_camera_loss(loss_dict, output_dict, input_dict, args, pred_dict):
    # context feat loss
    if args.enable_context_feat_loss:
        ...
        if args.feat_loss_type == 'mse':
            ...
            if "context_semantic_labels" in input_dict:
                ...
                # Compute similarity with temperature scaling (same as LSeg)
                similarity = (
                    valid_pred_feat @
                    get_text_label_feats(SEMANTIC_LABEL_LIST).T / 0.07
                )
                ...
                context_feat_loss = (
                    F.cross_entropy(similarity, valid_semantic_labels.long())
                    / valid_pred_feat.shape[0]
                    * 1.0
                )
            else:
            ...
            raise ValueError
        loss_dict["context_feat_loss"] = args.feat_loss_coeff * context_feat_loss

    # camera pose loss
    has_camera_pred = "pred_context_camera_enc_list" in output_dict
    has_camera_gt = "context_camtoworlds" in input_dict and "context_intrinsics" in input_dict
    if args.enable_camera_loss and has_camera_pred and has_camera_gt:
...
def _compute_rendering_loss(loss_dict, pred_rgb, target_rgb, lpips_loss, args):
    # rendering loss
    if lpips_loss is not None:
...
def _compute_target_depth_and_pseudo_loss(loss_dict, pred_dict, target_dict, args):
    if args.enable_depth_loss and "target_depth" in target_dict:
        ...
        depth_loss_value = compute_depth_loss(pred_depth, target_depth)
        loss_dict["depth_loss"] = depth_loss_value
    ...
    if args.enable_pseudo_depth_loss and "target_pseudo_depth" in target_dict:
        pred_depth = pred_dict[pred_dict["depth_key"]]
        target_pseudo_depth = target_dict["target_pseudo_depth"]
        target_pseudo_depth_conf = target_dict["target_pseudo_depth_conf"]
        target_depth = target_dict["target_depth"]
        pseudo_depth_loss = compute_pseudo_depth_loss_v2(
            pred_depth,
            target_pseudo_depth,
            target_depth,
            target_pseudo_depth_conf,
        )
        loss_dict["pseudo_depth_loss"] = args.pseudo_depth_coeff * pseudo_depth_loss


def _compute_reg_and_flow_loss(loss_dict, ctx: RegAndFlowLossContext):
    gs_params = ctx.gs_params
    device = ctx.device
    args = ctx.args
    pred_dict = ctx.pred_dict
    target_dict = ctx.target_dict
    data_iter_step = ctx.data_iter_step
    ...
    if args.enable_flow_loss and pred_dict["flow_key"] is not None and data_iter_step > args.flow_loss_start_iter:
        ...
        if nonzero_valid_mask.sum() > 0:
            ...
            forward_flow_loss = (
                (1 - nonzero_weight) * nonzero_forward_flow_loss
                + nonzero_weight * zero_forward_flow_loss
            )
        else:
...
def _compute_sky_losses(loss_dict, pred_dict, target_dict, args):
...
def _compute_feat_loss(loss_dict, pred_dict, target_dict, args):
    if args.enable_feat_loss:
        ...
        if args.feat_loss_type == 'mse':
            ...
            raise ValueError
        loss_dict["feat_loss"] = args.feat_loss_coeff * feat_loss


def compute_loss(output_dict, input_dict, target_dict, args=None, loss_ctx=None):
    if loss_ctx is None:
        loss_ctx = LossContext()
    lpips_loss = loss_ctx.lpips_loss
    data_iter_step = loss_ctx.data_iter_step
    ...
    _compute_context_rgb_loss(loss_dict, output_dict, input_dict, args, lpips_loss)
    _compute_context_sky_opacity_loss(loss_dict, output_dict, input_dict, args)
    _compute_context_depth_loss(loss_dict, output_dict, input_dict, args)
    _compute_context_point_loss(loss_dict, output_dict, input_dict, args)
    _compute_context_feat_and_camera_loss(loss_dict, output_dict, input_dict, args, pred_dict)

    # NOTE: Pre-train the 3D annotation head for the context part
    if data_iter_step < args.context_prediction_loss_warmup_steps:
        return loss_dict

    _compute_rendering_loss(loss_dict, pred_rgb, target_rgb, lpips_loss, args)
    _compute_target_depth_and_pseudo_loss(loss_dict, pred_dict, target_dict, args)
    _compute_reg_and_flow_loss(loss_dict, RegAndFlowLossContext(
        gs_params=gs_params, device=device, args=args,
        pred_dict=pred_dict, target_dict=target_dict, data_iter_step=data_iter_step,
    ))
    _compute_sky_losses(loss_dict, pred_dict, target_dict, args)
    _compute_feat_loss(loss_dict, pred_dict, target_dict, args)
...
def compute_scene_flow_metrics(pred, labels):
    ...
    # modified from:
    # https://github.com/Lilac-Lee/Neural_Scene_Flow_Prior/blob/0e4f403c73cb3fcd5503294a7c461926a4cdd1ad/utils.py#L12
    ...
    epe_3d = torch.mean(l2_norm).item()  # Mean absolute distance error
...
def compute_semantic_metrics(pred, labels, interested_classes=None, ignore_index=255, orig_img=None):
    if interested_classes is None:
        interested_classes = [1, 2, 3]

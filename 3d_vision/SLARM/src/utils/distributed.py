# coding=utf-8
# Adapted from
# https://github.com/NVlabs/GaussianSTORM/blob/main/storm/utils/distributed.py
...
def _parse_slurm_node_list(s: str) -> List[str]:
    ...
    for match in pattern.finditer(s):
        prefix = s[match.start(2):match.end(2)]
        suffixes = s[match.start(3):match.end(3)]
        for suffix in suffixes.split(","):
            ...
            if len(span) == 1:
                ...
                nodes.extend(
                    [prefix + f"{i:0{width}}" for i in range(start, end)]
                )
    return nodes
...
def _check_env_variable(key: str, new_value: str):
    ...
    if key in os.environ and os.environ[key] != new_value:
        raise RuntimeError(
            f"Environment variable conflict: {key} is already set"
        )
...
class _TorchDistributedEnvironment:
    ...
    def __init__(self):
        ...
        if not env_vars:
            ...
            unset_vars = [
                v for v in _TORCH_DISTRIBUTED_ENV_VARS if v not in env_vars
            ]
            raise RuntimeError(
                f"Partially set environment: {collected_env_vars}. "
                f"Unset environment variables: {unset_vars}"
            )
        ...
        raise RuntimeError(
            "Can't initialize PyTorch distributed environment"
        )
    ...
    def _set_from_slurm_env(self):
        ...
        if len(nodes) != node_count:
            raise RuntimeError(
                f"SLURM_JOB_NODELIST mismatch: {nodes} vs {node_count}"
            )

...
本项目欢迎广大开发者体验并参与贡献，在参与社区贡献之前。请参见[cann-community](https://gitcode.com/cann/community)了解行为准则，了解源码仓的贡献流程。
...
## 签署CLA
参与项目贡献前，请先确保根据您的贡献者身份完成相应的贡献者许可协议签署（CLA）。具体操作步骤请参阅：[CLA 使用指南](https://gitcode.com/cann/infrastructure/blob/main/docs/cla/cla%E4%BD%BF%E7%94%A8%E6%8C%87%E5%8D%97.md)。请注意：
- 签署CLA后使用签署邮箱提交您的贡献。
- 如果当前贡献来自多位贡献者，请确保每一位贡献者在提交commit前都已签署CLA。

## 新增样例要求

感谢您参与样例建设，为cann-recipes注入更多价值！为了让您的贡献过程更顺畅、方案能更好地契合社区需求，在完成了[CLA签署](#签署cla)后，以下是详细的操作指引，供您参考：
...
- 样例代码：请确保代码风格符合社区基本要求，且能通过流水线的code check，让后续开发者更容易理解和使用，请确保每一个PR仅包含一个样例；
...
### 样例目录结构参考
本仓库按照具身智能常用组件进行划分来组织目录结构，您可以参考以下结构组织您的代码和文档（样例名建议全部小写，多个单词之间用短中划线连接，比如spirit-v1.5）：
```
├── manipulation                                # 感知操作类模型样例目录
|  ├── sample1                                  # 您的样例名（如spirit-v1.5）
|  |   ├── train                                # 训练样例
...
|  ├── sample2
│  └── ...
├── locomotion                                  # 规划控制类模型样例目录
├── world_model                                 # 世界模型类模型样例目录
├── 3d_vision                                   # 3D视觉类模型样例目录
